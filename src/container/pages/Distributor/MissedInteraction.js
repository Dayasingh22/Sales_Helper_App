...
const MissedInteraction = () => {
    ...
    axios(config)
      .then(function(response) {
        ...
        distributors.map(user => {
          ...
          let callOpenDate = new Date(callOpen.callBackDate);
          let meetingProposedDate = new Date(meetingProposed.callBackDate);
          let revisit1Date = new Date(revisit1);
          let revisit2Date = new Date(revisit2);
          let meetingDoneDate = new Date(meetingDone);

          let callOpenDateDiff = new Date().getTime() - callOpenDate.getTime();
          let meetingProposedDateDiff = new Date().getTime() - meetingProposedDate.getTime();
          let revisit1DateDiff = new Date().getTime() - revisit1Date.getTime();
          let revisit2DateDiff = new Date().getTime() - revisit2Date.getTime();
          let meetingDoneDateDiff = new Date().getTime() - meetingDoneDate.getTime();
          if (
            callOpenDateDiff > 0 ||
            meetingProposedDateDiff > 0 ||
            revisit1DateDiff > 0 ||
            revisit2DateDiff > 0 ||
            meetingDoneDateDiff > 0
          ) {
            usersTableData.push({
              key: _id,
              name: name,
  ...
  return (
    <>
      <PageHeader title="Missed Interaction List" />
      <p style={{ marginLeft: '30px', color: 'red' }}>
        You have Total {usersData.length} Missed Interaction with Distributors.
      </p>
      <Main>
        <Row gutter={25}>
          <Col sm={24} xs={24}>
...
export default MissedInteraction;
